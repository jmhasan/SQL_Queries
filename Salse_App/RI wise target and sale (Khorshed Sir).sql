...
SUM(xqtychl)/20 xqtychl from opchallandt where xdatecom between  '2020-10-01' and '2020-10-09' 
...
xemp not in (select xteam from opchallan where xdatecom between  '2020-10-01' and '2020-10-09' and xdiv in ('Dhaka','Out Dhaka','Corporate') ))
select xdiv,xzone,xemp,(select xname from prmst where xemp=cte.xemp and zid=100000),xempnew,
(select xname from prmst where xemp=cte.xempnew and zid=100000),sum(xrow),sum(xqty),sum(xqtychl) from cte
