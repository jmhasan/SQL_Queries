...
(case when xtrngl in ('CHQ-','BR--','JVNA','CR--','CRF-','JVI-','JV--') and h.xdate between '2020-11-01' and '2020-11-01' then 
...
where xtrngl<>'OB--' and h.xpostflag='Posted' and c.xsimcardno='Corporate' and h.xdate <='2020-11-01 06:00:00.000') 
, cte2 as( 
...
xconfirmt between '2020-11-01 06:00:00.000' and '2020-11-01 06:00:00.000') salesqty, 
...
xconfirmt between '2020-11-01 06:00:00.000' and '2020-11-01 06:00:00.000') saleamount, 
--SUM(xqty) salesqty,SUM(saleamount) saleamount, 
SUM(collectionamount) collectionamount,SUM(xprime)balance from xyz 
group by zid,xsub) 
select xriid,(select xname from prmst where zid=100000 and xemp=cte2.xriid) xname , 
...
SUM(balance) balance,(SUM(balance)/SUM(saleamount))*30 from cte2 
