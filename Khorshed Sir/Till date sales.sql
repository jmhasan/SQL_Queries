...
d on h.zid=d.zid and h.xdornum=d.xdornum where xstatusdor<>'C-Cancelled' and xdate<='2020-11-16' 
...
CONVERT(date,DATEADD(HOUR,-6,xconfirmt))>'2020-11-16' group by h.zid,h.xcus 
union all 
select h.zid,h.xcus,0 DORemaining,sum(xqtychl/20) tillsales,sum(case when CONVERT(date,DATEADD(HOUR,-6,xconfirmt))='2020-11-16' 
...
where xstatuschl='3-Invoiced' and CONVERT(date,DATEADD(HOUR,-6,xconfirmt)) between '2020-11-01' and '2020-11-16' group by h.zid,h.xcus 
union all 
select zid,xcus,0 DORemaining,0 tillsales,0 ondsales,sum(xqtysms-xqtychl)/20 from opadvice where 
xstatusadvmach not in ('5-Voided','1-Open') and CONVERT(date,DATEADD(HOUR,-6,xconfirmt))<='2020-11-16' group by zid,xcus 
union all 
