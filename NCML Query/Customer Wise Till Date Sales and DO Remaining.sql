...
with opsalesdoncml  (xcus,xorg,Zone,tillsales,ondsales,DORemaining) as (
select c.xcus,c.xorg,c.xbloodgrp,sum(xqtychl/20) tillsales,
...
union all 
select c.xcus,c.xorg,c.xbloodgrp,0 tillsales,0 ondsales,sum(xqty-xqtychl)/20 DORemaining from opdor h join opddt d on h.zid=d.zid and h.xdornum=d.xdornum 
join cacus c on c.zid=h.zid and c.xcus=h.xcus
where xstatusdor<>'C-Cancelled' and h.xdate<=@txdate  group by c.xcus,c.xorg,c.xbloodgrp having sum(d.xqty-d.xqtychl)>0)

select xcus,xorg,Zone,sum(tillsales) tillsales,sum(ondsales) ondsales,sum(DORemaining) DORemaining from opsalesdoncml group by xcus,xorg,Zone
